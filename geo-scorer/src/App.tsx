...
async function fetchPageSpeedData(url: string): Promise<Partial<Record<string, number>>> {
  try {
    ...
    // STRUCTURED_DATA — overall SEO score
    const seoScore = data.lighthouseResult?.categories?.seo?.score;
    if (seoScore !== undefined) {
      overrides.STRUCTURED_DATA = jitter(Math.round(seoScore * 90), 8);
    }

    // SEMANTIC_HTML — heading order audit
    ...
    // CONTENT_MATCH — title + meta description presence
    ...
    // ENTITY_CLARITY — structured-data audit from Lighthouse
    ...
    // DIRECT_ANSWER — check for FAQ schema in structured data details
...
async function fetchAllRealData(url: string): Promise<Partial<Record<string, number>>> {
  const [pagespeed, wikipedia, llmstxt, freshness] = await Promise.allSettled([
    ...
    fetchWikipediaAuthority(url),
    fetchLLMSTxt(url),
    fetchContentFreshness(url),
  ]);
  return {
  ...
  return (
    <div className="app">
      ...
      <header className="site-header">
        ...
        <div className="header-right">
          ...
          <button className="connect-btn">
            <span className="connect-dot" />Connect your agent
          </button>
      ...
      <section className="hero-section">
        <motion.div className="hero-eyebrow" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          Generative Engine Optimization
        </motion.div>
        <motion.h1 className="hero-title" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          Assess your brand’s visibility<br />to drive AI-led positioning
        </motion.h1>
      ...
      {!hasResults && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
          <UrgencyBlock sector={sector} />
        </motion.div>
      )}

      <motion.div className="mode-tabs" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
        <button className={`mode-tab ${mode === 'analyze' ? 'active' : ''}`}
          onClick={() => { setMode('analyze'); setResultsA(null); setResultsB(null); }}>
        ...
        <button className={`mode-tab ${mode === 'compare' ? 'active' : ''}`}
          onClick={() => { setMode('compare'); setResultsA(null); setResultsB(null); }}>
          <span className="tab-icon">⊕</span>
          ...
          {mode === 'analyze' ? (
            <motion.div key="a" className="input-row" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
              <div className="input-wrap">
                ...
                <input type="text" placeholder="https://yourbrand.com" value={urlA}
                  onChange={e => setUrlA(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAnalyze()} />
              </div>
            ...
            <motion.div key="b" className="input-row compare-input-row" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
              <div className="compare-inputs">
                ...
                <div className="input-wrap">
                  ...
                  <input type="text" placeholder="https://competitor.com" value={urlB}
                    onChange={e => setUrlB(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAnalyze()} />
                </div>
              </div>
              <button className="cta-btn" onClick={handleAnalyze} disabled={loading || selectedLLMs.length === 0}>
        ...
        {loading && (
          <motion.div className="loading-state" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <p className="loading-subtext">
              Running across {selectedLLMs.length} LLM{selectedLLMs.length > 1 ? 's' : ''}: {LLMS.filter(l => selectedLLMs.includes(l.key)).map(l => l.label).join(', ')}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {hasResults && !loading && (
          <div className="results-area">
            ...
            {mode === 'compare' && resultsA && resultsB && (
              <ResultsPanel url={analyzedA} results={resultsA} compareUrl={analyzedB} compareResults={resultsB}
                sector={sector} selectedLLMs={selectedLLMs} onShowBadge={() => setShowBadge(true)} />
            )}
          </div>
        )}
      </AnimatePresence>
            ...
            {['Entity & Authority', 'Content & Semantics', 'Technical & Visibility'].map((label, i) => (
              <div key={i} className="empty-card">
                ...
                <div className="empty-bars">
                  {[70, 45, 85, 30].map((w, j) => (
                    <div key={j} className="empty-bar" style={{ width: `${w}%`, opacity: 0.15 + j * 0.07 }} />
                  ))}
                </div>
              </div>
            ))}
      ...
      <footer className="site-footer">
        <span>GEO Auditor · {new Date().getFullYear()}</span>
        ...
        {showHistory && (
          <>
            ...
            <HistoryPanel history={history} onClose={() => setShowHistory(false)}
              onSelect={entry => { setUrlA(entry.url); setSector(entry.sector); }} />
          </>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showBadge && resultsA && (
          <BadgeModal score={overall} url={analyzedA} onClose={() => setShowBadge(false)} />
        )}
      </AnimatePresence>
